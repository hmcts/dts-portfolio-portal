...
test("/upload renders the 'Download a starter template' section", async ({
  page,
}) => {
...
test("the page has three Download template links, one per template, with the right hrefs", async ({
  page,
}) => {
  await page.goto("/upload");
...
test("the template files are actually served from /templates/", async ({
  request,
}) => {
